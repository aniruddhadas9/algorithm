...
public class BinarySearch {
	...
	public static boolean search(List<Integer> numbers, int key) {
		...
		int median = (int) numbers.get(numbers.size() / 2);
		...
		if (median == key) {
			...
			search((List<Integer>) numbers.subList(medianIndex, numbers.size()), key);
		} else if (median > key) {
			search((List<Integer>) numbers.subList(0, medianIndex), key);
		}
		return returnValue;
